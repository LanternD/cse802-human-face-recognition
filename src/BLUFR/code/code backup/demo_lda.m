%% This demo shows how to apply LDA dimension reduction along with 
...
outLogFile = [outDir, strcat('result_lfw_lda_', file_name, '.mat')]; % output text file
...
%     W = mapping.M;
    disp(size(W))
%     disp(size(mapping_lda))
    % Get the test data of the t'th trial
    X = Descriptors(testIndex{t}, :);
    
    % Transform the test data into the learned LDA subspace of ldaDims dimensions.
    X = X * W(:, 1 : ldaDims);
